...
var make_ssf = function(SSF) {
	...
	function pad(v, d, c) {
		var t = String(v);
		return t.length >= d ? t : (fill(c || 0, d - t.length) + t);
	}

	function rpad(v, d, c) {
		var t = String(v);
		return t.length >= d ? t : (t + fill(c || 0, d - t.length));
	}
	...
	var general_fmt = function(v) {
		...
		if ( typeof v === 'number') {
			...
			if (V >= 0.1 && V < 1)
				o = v.toPrecision(9);
			else if (V >= 0.01 && V < 0.1)
				o = v.toPrecision(8);
			else if (V >= 0.001 && V < 0.01)
				o = v.toPrecision(7);
			else if (V >= 0.0001 && V < 0.001)
				o = v.toPrecision(6);
			else if (V >= Math.pow(10, 10) && V < Math.pow(10, 11))
				o = v.toFixed(10).substr(0, 12);
			else if (V > Math.pow(10, -9) && V < Math.pow(10, 11)) {
				o = v.toFixed(12).replace(/(\.[0-9]*[1-9])0*$/, "$1").replace(/\.$/, "");
				if (o.length > 11 + (v < 0 ? 1 : 0))
					o = v.toPrecision(10);
				if (o.length > 11 + (v < 0 ? 1 : 0))
					o = v.toExponential(5);
			} else {
	...
	var write_num = function(type, fmt, val) {
		...
		if (mul !== 0)
			return write_num(type, fmt, val * Math.pow(10, 2 * mul)) + fill("%", mul);
		if (fmt.indexOf("E") > -1) {
			...
			if (fmt.match(/E\+00$/) && o.match(/e[+-][0-9]$/))
		...
		if (fmt[0] === "$")
			return "$" + write_num(type, fmt.substr(fmt[1] == ' ' ? 2 : 1), val);
		...
		if (fmt.match(/^00*$/))
			return (val < 0 ? "-" : "") + pad(Math.round(Math.abs(val)), fmt.length);
		if (fmt.match(/^####*$/))
			return "dafuq";
		switch(fmt) {
			case "0":
			...
			case "#,##0.0":
				r = Math.round((val - Math.floor(val)) * 10);
				return val < 0 ? "-" + write_num(type, fmt, -val) : commaify(String(Math.floor(val))) + "." + r;
			case "#,##0.00":
				r = Math.round((val - Math.floor(val)) * 100);
				return val < 0 ? "-" + write_num(type, fmt, -val) : commaify(String(Math.floor(val))) + "." + (r < 10 ? "0" + r : r);
			case "# ? / ?":
				ff = frac(aval, 9, true);
				return sign + (ff[0] || "") + " " + (ff[1] === 0 ? "   " : ff[1] + "/" + ff[2]);
			case "# ?? / ??":
				ff = frac(aval, 99, true);
				return sign + (ff[0] || "") + " " + (ff[1] ? pad(ff[1], 2, " ") + "/" + rpad(ff[2], 2, " ") : "     ");
			case "# ??? / ???":
				ff = frac(aval, 999, true);
				return sign + (ff[0] || "") + " " + (ff[1] ? pad(ff[1], 3, " ") + "/" + rpad(ff[2], 3, " ") : "       ");
			default:
		}
	...
	function split_fmt(fmt) {
		...
		if (in_str != -1)
			throw "Format |" + fmt + "| unterminated string at " + in_str;
		return out;
	}
	...
	function choose_fmt(fmt, v, o) {
		...
		switch(fmt.length) {
			case 1:
				fmt = [fmt[0], fmt[0], fmt[0], "@"];
				break;
			case 2:
				fmt = [fmt[0], fmt[fmt[1] === "@" ? 0 : 1], fmt[0], "@"];
				break;
			case 4:
				break;
			default:
...
(function(XLSX) {
	...
	function parseSheet(data) {
		...
		if (ref && ref.length == 2 && ref[1].indexOf(":") !== -1)
			s["!ref"] = ref[1];
		...
		if (!data.match(/<sheetData *\/>/))
			data.match(/<sheetData>([^\u2603]*)<\/sheetData>/m)[1].split("</row>").forEach(function(x) {
				...
				if (refguess.s.r > row.r - 1)
					refguess.s.r = row.r - 1;
				if (refguess.e.r < row.r - 1)
					refguess.e.r = row.r - 1;
				...
				cells.forEach(function(c, idx) {
					...
					if (refguess.e.c < idx)
						refguess.e.c = idx;
					var cell = parsexmltag((c.match(/<c[^>]*>/)||[c])[0]);
					delete cell[0];
					...
					switch(p.t) {
						...
						case 'inlineStr':
							p.t = 'str';
							p.v = unescapexml((d.match(matchtag('t'))||["",""])[1]);
							break;
						// inline string
						case 'b':
							switch(p.v) {
		...
		(data.match(/<[^>]*>/g) || []).forEach(function(x) {
			...
			switch(y[0]) {
				...
				case '<Default':
					...
					break;
				case '<Override':
					if (y.ContentType in ct2type)
						ct[ct2type[y.ContentType]].push(y.PartName);
					break;
			}
		});
		if (ct.xmlns !== XMLNS_CT)
	...
	function parseWB(data) {
		...
		if (wb.xmlns !== XMLNS_WB)
			throw new Error("Unknown Namespace: " + wb.xmlns);

		var z;
		/* defaults */
		for (z in WBPropsDef)
		if ( typeof wb.WBProps[z] === 'undefined')
			wb.WBProps[z] = WBPropsDef[z];
		for (z in CalcPrDef)
		if ( typeof wb.CalcPr[z] === 'undefined')
			wb.CalcPr[z] = CalcPrDef[z];
	...
	function parseStyles(data) {
		...
		/* numFmts CT_NumFmts ? */
		if (( t = data.match(/<numFmts([^>]*)>.*<\/numFmts>/)))
			parseNumFmts(t);
		...
		/* cellXfs CT_CellXfs ? */
		if (( t = data.match(/<cellXfs([^>]*)>.*<\/cellXfs>/)))
			parseCXfs(t);
	...
	function getdata(data) {
		...
		if (data._data && data._data.getContent)
			return Array.prototype.slice.call(data._data.getContent(), 0).map(function(x) {
	...
	function parseZip(zip) {
		...
		if (dir.workbooks.length === 0)
			throw new Error("Could not find workbook entry");
		strs = {};
		if (dir.sst)
			strs = parse_sst(getdata(getzipfile(zip, dir.sst.replace(/^\//, ''))));

		styles = {};
		if (dir.style)
			styles = parseStyles(getdata(getzipfile(zip, dir.style.replace(/^\//, ''))));
		...
		var deps = {};
		if (dir.calcchain)
			deps = parseDeps(getdata(getzipfile(zip, dir.calcchain.replace(/^\//, ''))));
		var sheets = {}, i = 0;
		if (!props.Worksheets) {
...
function sheet_to_csv(sheet) {
	var stringify = function stringify(val) {
		switch(val.t) {
			...
			case 'e':
				return "";
			/* throw out value in case of error */
			default:
				throw 'unrecognized type ' + val.t;
		}
	};
