...
(function make_xls(XLS) {
	...
	var CFB = (function _CFB() {
		...
		function make_find_path(FullPaths, Paths, FileIndex, files, root_name) {
			...
			for ( i = 0; i < FullPaths.length; ++i)
				UCFullPaths[i] = FullPaths[i].toUpperCase();
			for ( i = 0; i < Paths.length; ++i)
				UCPaths[i] = Paths[i].toUpperCase();
			...
			for (; i < sector.length; i += 128) {
				...
				if (o.type === 5) {/* root */
					...
					if (sector_list[o.start] === undefined)
						if ((o.start += dir_start) >= sectors.length)
							o.start -= sectors.length;
		...
		function readSync(blob, options) {
			switch(options !== undefined && options.type !== undefined ? options.type : "base64") {
				...
				case "base64":
					return parse(s2a(Base64.decode(blob)));
				case "binary":
	...
	function parse_BOF(blob, length) {
		...
		length -= 2;
		if (o.BIFFVer !== 0x0600 && o.BIFFVer !== 0x0500)
	...
	var crypto_CreateXorArray_Method1 = (function() {
		...
		return function(password) {
			...
			if ((Index & 1) === 1) {
				Temp = XorKey >> 8;
				ObfuscationArray[Index] = XorRor(PadArray[0], Temp); --Index;
				Temp = XorKey & 0xFF;
			...
			while (Index > 0) {--Index;
				Temp = XorKey >> 8;
				ObfuscationArray[Index] = XorRor(Password[Index], Temp); --Index;
				Temp = XorKey & 0xFF;
			...
			while (PadIndex > 0) {
				Temp = XorKey >> 8;
				ObfuscationArray[Index] = XorRor(PadArray[PadIndex], Temp); --Index; --PadIndex;
				Temp = XorKey & 0xFF;
				ObfuscationArray[Index] = XorRor(Password[Index], Temp); --Index; --PadIndex;
			}
	...
	var fixopts = fix_opts_func([['cellNF', false], /* emit cell number format string as .z */
	...
	function parse_compobj(obj) {
		...
		if (( m = __readUInt32LE(o, l)) !== 0x71b2e9f4)
			return v;
